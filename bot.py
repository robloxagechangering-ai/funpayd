...
BOT_USERNAME = "secretariOffreybot"  # Твой юзернейм (без @)
...
async def cmd_novateam(message: Message):
    ...
    cur.execute("SELECT * FROM deals WHERE seller_id = ? AND status = 'waiting_payment' ORDER BY created_at DESC LIMIT 1", (user_id,))
    deal = cur.fetchone()
...
Продавец свяжется с вами для передачи товара."""
        await bot.send_message(chat_id=buyer_id, text=buyer_text)
